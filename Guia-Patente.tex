...
% ── 2. Aviso legal ───────────────────────────────────────────────────────────
\msec{Aviso legal (pendiente con asesoría)}{Legal notice (open item, pending counsel)}
\noticebox{%
{\small\textbf{\color{warnbd!60!black}ES\ \ }El paquete incluye por ahora la \texttt{LICENSE} vigente (\textbf{MIT}) y el código ya fue publicado públicamente (GitHub público; \texttt{epiforecast.mx} en vivo). Una publicación MIT abierta es \textbf{divulgación habilitante} y concede licencia irrevocable de uso/venta, lo que puede minar la novedad en jurisdicciones de novedad absoluta (MX, EPO). \textbf{Es una decisión de gobernanza para el asesor legal/PI, no un defecto del código.}}\\[5pt]
{\small\textbf{\color{warnbd!60!black}EN\ \ }\itshape The bundle currently ships the existing \texttt{LICENSE} (\textbf{MIT}) and the source was already publicly published. An open MIT release is an \textbf{enabling public disclosure} and grants an irrevocable use/sell license, which can undermine novelty in absolute-novelty jurisdictions. \textbf{This is a governance decision for legal/IP counsel, not a code defect.}}}

% ── 3. Estructura: carpeta y componente ──────────────────────────────────────
...
\draw[climflow] (enso.south) .. controls +(0,-1.3) and +(1.4,0.7) .. (e1.north east);
% Stage 3
...
\foreach \e in {e1,e2,e3,e4,e5} {\draw[flow, draw=indigo!35] (\e.south) -- ([yshift=3mm]s2.north);}
% Stage 4
...
\draw[flow] (s1.south) -- (o1.north);
\draw[flow] (s2.south) -- (o2.north);
\draw[flow] (s3.south) -- (o3.north);
% Panels (background)
...
\end{center}
\vspace{-2pt}
{\footnotesize\color{soft}El método es \emph{config-driven} (\texttt{config/}); el regresor de El Niño (ENSO/ONI, líneas punteadas) alimenta a NBGLM y Prophet. \textit{The method is config-driven; the El Niño regressor (dashed) feeds NBGLM and Prophet.}}
