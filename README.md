...
## ⚠️ LEGAL NOTICE / AVISO LEGAL (open item — pending counsel)

**EN —** This bundle currently ships the repository's existing `LICENSE` (**MIT**), and the source has been **publicly published** (`github.com/.../EpiForecast-MX`, public; `epiforecast.mx` live). An open MIT release is an **enabling public disclosure** and grants an irrevocable use/sell license, which can undermine patent novelty in absolute-novelty jurisdictions (MX, EPO). **This is a governance decision for legal/IP counsel, not a code issue.** Before any patent filing, confirm the license posture (e.g., replace MIT with an "All Rights Reserved / Confidential" notice aligned across `LICENSE`, `pyproject.toml`, and `README.md`) and audit the disclosure timeline against the 12-month grace period (US 35 U.S.C. §102(b)(1); MX). See the audit report `reports/AUDITORIA_PATENTE_CODIGO.md`, §4.

**ES —** Este paquete incluye por ahora la `LICENSE` vigente del repositorio (**MIT**) y el código ya fue **publicado públicamente** (GitHub público; `epiforecast.mx` en vivo). Una publicación MIT abierta es **divulgación habilitante** y concede licencia irrevocable de uso/venta, lo que puede minar la novedad en jurisdicciones de novedad absoluta (MX, EPO). **Es una decisión de gobernanza para el asesor legal/PI, no un defecto de código.** Antes de cualquier trámite, confirmar la postura de licencia (p. ej. sustituir MIT por un aviso "Todos los derechos reservados / Confidencial" alineado en `LICENSE`, `pyproject.toml` y `README.md`) y auditar las fechas de divulgación contra la gracia de 12 meses. Ver el informe de auditoría `reports/AUDITORIA_PATENTE_CODIGO.md`, §4.
