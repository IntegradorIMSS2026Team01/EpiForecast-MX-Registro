"""Genera paneles individuales 2x2 con zoom desde 2020.

Replica exacta de paneles_individuales pero con xlim desde 2020-01-01.
Salida: reports/forecasts/comparacion_modelos/{Pad}/paneles_individuales_zoom/
"""
...
from epiforecast.visualization.comparison_builders import build_small_multiples
...
def main() -> None:
    ...
    for (pad_, ent_, modo_), _ in grupos:
        ...
        # Generar panel 2x2
        fig = build_small_multiples(serie_real, target_y, groups, pad, ent, modo)

        # Aplicar zoom desde 2020 en los 4 subplots
        ...
        zoom_dir = output_dir / pad_norm / "paneles_individuales_zoom"
        directory_manager.asegurar_ruta(zoom_dir)
        fig.savefig(zoom_dir / base_name, dpi=VIZ_DPI_SCREEN, bbox_inches="tight")
        plt.close(fig)
        count += 1

        if count % 50 == 0:
            logger.info("Generados {} paneles zoom...", count)

    logger.success("Total: {} paneles zoom generados en {}", count, output_dir)
